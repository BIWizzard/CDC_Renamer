...
const AppContent: React.FC = () => {
  ...
  const [regexPatternError, setRegexPatternError] = useState<string | null>(
    null
  );
  ...
  const loadFiles = async () => {
    ...
    try {
      ...
      const filesList = fileNames.map((fileName: string) => {
        ...
        const baseName = originalBase.replace(
          new RegExp(regexPattern.replace("\\.txt$", "")),
          ""
        );
  ...
  const renameFiles = async () => {
    ...
    const confirmRename = window.confirm(
      `This will rename ${files.length} files. Proceed?`
    );
    if (!confirmRename) {
      return;
    }
    ...
    try {
      ...
      const script = `
        ...
        Get-ChildItem -Path "${sourceDir}" -Filter "*.txt" | ForEach-Object {
            ...
            $baseName = $originalBase -replace "${regexPattern.replace(
              "\\.txt$",
              ""
            )}", ""
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
              ...
              <div className="options-container">
                  ...
                  <div className="checkbox-container">
                    <input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                        setPreviewOnly(e.target.checked)
                      }
                      id="previewCheckbox"
                    />
                ...
                <div className="options-right">
                  <label>
                    ...
                    <span
                      className="info-icon"
                      title="This pattern identifies the timestamp portion of filenames that should be replaced. The default pattern matches date_time format like '_2025-04-25_14-30-00'."
                    >
                      i
                    </span>
                  </label>
                  <input
                    ...
                    className={`input-field w-full ${
                      regexPatternError ? "border-red-500" : ""
                    }`}
                    value={regexPattern}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      handleRegexPatternChange(e.target.value)
                    }
                  />
                  {regexPatternError && (
            ...
            <div className="button-group">
              <button
                className={`button button-blue ${
                  showLoadingAnimation && isLoading ? "loading-button" : ""
                }`}
                onClick={loadFiles}
            ...
            <div className="status-container">
              <div
                className={`status-message ${
                  statusMessage.includes("Successfully") ? "status-success" : ""
                } ${showSuccessAnimation ? "successFlash" : ""}`}
              >
                {statusMessage}
              </div>
            </div>
      ...
      {showLoadingAnimation && (
        <div
          className={`progress-bar ${isLoading ? "progress-animate" : ""}`}
        ></div>
      )}
    </div>
  );
